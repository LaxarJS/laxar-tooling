...
exports.create = function( log, config ) {
   ...
   const readFile = config.readFile || require( './file_reader' ).create( log, fileContents );
   const fileExists = config.fileExists || function( file ) {
...
function artifactProcessor( listings, themes, options ) {
   ...
   function embed( filePath ) {
      ...
      return readFile( filePath, 'utf-8' )
         ...
         .then( function( contents ) {
            insertRecursively( listings, filePath.split( path.sep ), contents );
         } )
         .catch( function( err ) {

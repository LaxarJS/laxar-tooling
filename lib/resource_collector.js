...
exports.create = function( log, config ) {
   ...
   var readFile = config.readFile || require( './file_reader' ).create( log, fileContents );
   var fileExists = config.fileExists || function( file ) {
      return promise.nfcall( fs.access, file, fs.F_OK ).then(

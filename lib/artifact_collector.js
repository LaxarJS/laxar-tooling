...
exports.create = function( log, config ) {
   ...
   const readJson = config.readJson || require( './json_reader' ).create( log, fileContents );

   const projectPath = config.projectPath || Promise.resolve;
   const projectRef = config.projectRef || function( ref ) {
   ...
   function collectPages( flows ) {
      ...
      function followPage( pageRef ) {
         var resolvedPagePromise = resolveArtifact( pageRef, 'laxar-path-pages', {
            defaultScheme: 'local',
            readDescriptor: pagePath => readJson( pagePath + '.json' )
         } );
      ...
      function followWidget( widgetRef ) {
         ...
         return resolvedWidgetPromise
            .then( function( resolvedWidget ) {
               ...
               const nameFromDirectory = resolvedWidget.project.path.split( URL_SEP ).pop();

               return fileExists( path.join( resolvedWidget.project.path, nameFromDirectory + '.js' ) )
                  .then( function( nameFromDirectoryIsValid ) {
                     resolvedWidget.module.name = selectName( resolvedWidget.module.name, nameFromDirectory, nameFromDirectoryIsValid );
   ...
   function collectControls( widgets, themes ) {
      ...
      function followControl( controlRef ) {
         ...
         function fakeDescriptor() {
            ...
            const nameFromDirectory = controlRef.split( URL_SEP ).pop();

...
var glob = helpers.nfbind( require( 'glob' ) );
var readdir = helpers.nfbind( fs.readdir );
var stat = helpers.nfbind( fs.stat );
...
exports.create = function( log, config ) {
   ...
   var projectPath = config.projectPath || identity;
   var projectRef = config.projectRef || function( ref ) {
      return path.join( 'laxar-application', projectPath( ref ) );
   };
   ...
   function collectArtifacts( flowPaths ) {
      var flowsPromise = collectFlows( flowPaths );
      var themesPromise = collectThemes();
      var pagesPromise = flowsPromise.then( collectPages );
      ...
      return Promise.all( [ flowsPromise, themesPromise, pagesPromise ] )
         ...
         .then( function( artifacts ) {
            ...
            return Promise.all( [ layoutsPromise, widgetsAndControlsPromise ] ).then( function( results ) {
   ...
   function collectPages( flows ) {
      ...
      return Promise.all( flows.map( followFlowToPages ) ).then( helpers.flatten );
      ...
      function followFlowToPages( flowInfo ) {
         return readJson( flowInfo.path ).then( function( flow ) {
            return Promise.all( values( flow.places ).map( followPlaceToPages ) ).then( helpers.flatten );
         } );
      }
   ...
   function collectWidgets( pages, themes ) {
      ...
      function followPageToWidgets( page ) {
         return Promise.all( page.widgets.map( followWidgetOnce ) ).then( helpers.flatten );
      }
      ...
      function followWidget( widgetRef ) {
         ...
         return Promise.all( [
            ...
            return [ {
               ...
               references: {
                  local: {
                     self: path.relative( projectPath( 'laxar-path-widgets' ), widgetPath )
                  },
                  amd: {
                     self: path.relative( projectPath( '.' ), widgetPath ),
   ...
   function collectControls( widgets, themes ) {
      ...
      function followWidgetToControls( widget ) {
         return Promise.all( widget.controls.map( followControlOnce ) ).then( helpers.flatten );
      }
         ...
         return readJson( descriptorPath )
            ...
            .then( function( descriptor ) {
               ...
               return [ {
                  ...
                  references: {
                     local: {
                        self: path.relative( projectPath( 'laxar-path-controls' ), controlPath )
                     },
                     amd: {
                        self: path.relative( projectPath( '.' ), controlPath ),
                        module: isAmdReference ? path.join( referenceInfo.ref, name ) :
                           projectRef( path.join( 'laxar-path-controls', referenceInfo.ref, name ) )
                     }
                  },
                  referencedAs: {
                     byUser: controlRef,
   ...
   function collectThemes() {
      var defaultThemePath = projectPath( 'laxar-path-default-theme' );
      var themesRootPath = projectPath( 'laxar-path-themes' );
   ...
   function collectLayouts( themes ) {
      ...
      function collectLayoutRefsFromApplication() {
         var layoutsRootPath = projectPath( 'laxar-path-layouts' );
      ...
      function followLayout( layoutRef ) {
         var layoutsPath = projectPath( 'laxar-path-layouts' );

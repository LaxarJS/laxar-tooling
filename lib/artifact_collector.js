...
exports.create = function( log, options ) {
   ...
   function collectWidgets( pages, themes ) {
      ...
      function patchWidgetName( widgetPath ) {
         return function( descriptor ) {
            ...
            return fileExists( path.join( widgetPath, nameFromDirectory + '.js' ) )
               .then( function( controlModuleExists ) {
                  descriptor.name = selectName( descriptor.name, nameFromDirectory, controlModuleExists );
                  return descriptor;
               } );
         };
      }
      ...
      function selectName( nameFromDescriptor, nameFromDirectory, useNameFromDirectory ) {
         if( useNameFromDirectory && nameFromDirectory !== nameFromDescriptor ) {
            ...
            handleDeprecation( title + ' ' + message + '\n' + details );
         }
   ...
   function checkForDuplicateModules( artifacts ) {
      ...
      if( duplicatePaths.length > 0 ) {
         log.warn( 'The following artifacts are referenced more than once using different names:' );
         duplicatePaths.forEach( function( path ) {
            const modules = duplicates[ path ].reduce( function( acc, artifact ) {

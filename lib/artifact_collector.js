...
exports.create = function( log, options ) {
   ...
   function collectPages( flows ) {
      ...
      function followPage( pageRef ) {
         const resolvedPagePromise = resolveArtifact( pageRef, 'laxar-path-pages', {
            defaultScheme: 'local',
            descriptorPath: pageRef => pageRef + '.json',
            artifactPath: descriptorPath => descriptorPath.replace( /\.json$/, '' )
         } );
   ...
   function collectWidgets( pages, themes ) {
      ...
      function followWidget( widgetRef ) {
         ...
         const resolvedWidgetPromise = resolveArtifact( widgetRef, 'laxar-path-widgets', {
            defaultScheme: 'local',
            descriptorPath: widgetRef => path.join( widgetRef, 'widget.json' ),
            artifactPath: path.dirname
         } );
            ...
            .then( function( resolvedWidget ) {
               ...
               return [ {
                  ...
                  references: {
                     ...
                     amd: {
                        ...
                        module: isLocalReference ?
                           path.join( resolvedWidget.local.ref, moduleName ) :
                  ...
                  referencedAs: {
                     ...
                     derivedReference: isLocalReference ? widgetRef : resolvedWidget.module.ref
                  }
               } ];
            } );
   ...
   function collectControls( widgets, themes ) {
      ...
      function followControl( controlRef ) {
         ...
         const resolvedControlPromise = resolveArtifact( controlRef, 'laxar-path-controls', {
            defaultScheme: 'amd',
            descriptorPath: controlRef => path.join( controlRef, 'control.json' ),
            artifactPath: path.dirname
         } );
               ...
               return [ {
                  ...
                  references: {
                     ...
                     amd: {
                        ...
                        module: isLocalReference ?
                           path.join( resolvedControl.local.ref, moduleName ) :
   ...
   function resolveArtifact( artifactRef, lookupPathRef, options ) {
      ...
      const resolveDescriptorPath = options.descriptorPath || ( artifactRef => artifactRef );
      const resolveArtifactPath = options.artifactPath || ( descriptorPath => descriptorPath );
      const readDescriptor = options.readDescriptor || readJson;
      ...
      const descriptorPathPromise = lookup( resolveDescriptorPath( ref ), lookupPathRef );
      const descriptorPromise = descriptorPathPromise.then( readDescriptor );
      const artifactPathPromise = descriptorPathPromise.then( resolveArtifactPath );

      return Promise.all( [ projectRootPromise, artifactPathPromise,
                            lookupPathPromise, descriptorPromise ] )
         .then( function( results ) {
            const projectRoot = results[ 0 ];
            const artifactPath = results[ 1 ];
            const lookupPath = results[ 2 ];
            ...
            return {
               ...
               local: {
                  ...
                  ref: path.join( 'laxar-application', artifactPath )
               },
               module: {
                  name,

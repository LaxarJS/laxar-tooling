...
const { flatten, values, identity } = utils;

var glob = promise.nfbind( require( 'glob' ) );
var readdir = promise.nfbind( fs.readdir );
var stat = promise.nfbind( fs.stat );
...
exports.create = function( log, config ) {
   ...
   var readJson = config.readJson || require( './json_reader' ).create( log, fileContents );

   var projectPath = config.projectPath || Promise.resolve;
   var projectRef = config.projectRef || function( ref ) {
      return projectPath( ref ).then( result => path.join( 'laxar-application', result ) );
   };
   ...
   function collectArtifacts( flowPaths ) {
      ...
      return Promise.all( [ flowsPromise, themesPromise, pagesPromise ] )
         .then( ( [ flows, themes, pages ] ) => ( { flows, themes, pages } ) )
         .then( ( { flows, themes, pages } ) => {
   ...
   function collectWidgets( pages, themes ) {
      ...
      function followWidget( widgetRef ) {
         ...
         return resolveWidget( widgetRef ).then( function( widgetPath ) {
            var descriptorPath = path.join( widgetPath, 'widget.json' );

            return Promise.all( [
               readJson( descriptorPath ),
               Promise.resolve( widgetPath ),
               fileExists( path.join( widgetPath, nameFromDirectory + '.js' ) )
            ] );
         } ).then( function( [ descriptor, widgetPath, nameFromDirectoryIsValid ] ) {
            ...
            return Promise.all( [ projectPath( '.' ),
                                  projectPath( 'laxar-path-widgets' ),
                                  projectRef( path.join( 'laxar-path-widgets', widgetRef, moduleName ) ) ] )
               .then( function( [ projectRoot, widgetsPath, widgetModule ] ) {
                  return {
                     local: {
                        self: path.relative( widgetsPath, widgetPath )
                     },
                     amd: {
                        self: path.relative( projectRoot, widgetPath ),
   ...
   function collectControls( widgets, themes ) {
      ...
      function followControl( controlRef ) {
         ...
         return resolveControl( controlRef )
            .then( function( controlPath ) {
               ...
               return Promise.all( [
                  readJson( descriptorPath ),
                  Promise.resolve( controlPath )
               ] ).then( identity, fakeDescriptor );
            } ).then( function( [ descriptor, controlPath ] ) {
               var referenceInfo = extractScheme( controlRef, 'amd' );
               var isAmdReference = referenceInfo.scheme === 'amd';
               var moduleName = artifactNameFromDescriptor( descriptor );
               ...
               var toWatch = toList.concat( [ moduleName + '.js' ] ).concat( resources.watch || [] );

               // Take into account possible theme-folder files
               themes.filter( nonDefault ).forEach( function( theme ) {
                  var basePart = [ '', theme.path, 'controls', moduleName ];
                  var themeCssPath = basePart.concat( cssPart ).join( path.sep );
                  toList.unshift( themeCssPath );
                  toWatch.unshift( themeCssPath );
               } );
               ...
               return Promise.all( [ projectPath( '.' ),
                                     projectPath( 'laxar-path-controls' ),
                                     projectRef( path.join( 'laxar-path-controls', referenceInfo.ref, moduleName ) ) ] )
                  .then( function( [ projectRoot, controlsPath, controlModule ] ) {
                     return {
                        local: {
                           self: path.relative( controlsPath, controlPath )
                        },
                        amd: {
                           self: path.relative( projectRoot, controlPath ),
                           module: isAmdReference ? path.join( referenceInfo.ref, moduleName ) :
                              controlModule
                        }
                     };
   ...
   function collectThemes() {
      return Promise.all( [ projectPath( 'laxar-path-default-theme' ),
         ...
         .then( function( [ defaultThemePath, themesRootPath ] ) {
   ...
   function resolvePage( pageRef ) {
      return projectPath( path.join( 'laxar-path-pages', pageRef + '.json' ) );
   }

   function resolveWidget( widgetRef ) {
      return projectPath( resolveAssetPath( widgetRef, 'laxar-path-widgets', 'local' ) );
   }

   function resolveControl( controlRef ) {
      return projectPath( resolveAssetPath( controlRef, 'laxar-path-controls' ) );
   }
